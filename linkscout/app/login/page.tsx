...
export default function LoginPage() {
    ...
    try {
      if (!isLogin) {
        ...
        if (error) {
          ...
          setMessage({ type: "success", text: "Compte créé ! Vérifiez votre e-mail pour confirmer avant de vous connecter." });
        }
      } else {
  ...
  const handleMagicLink = async () => {
    if (!email.trim()) {
      setMessage({ type: "error", text: "Veuillez entrer votre adresse e-mail." });
      return;
    }
    setLoading(true);
    setMessage(null);
    try {
      ...
      if (error) {
        ...
        setMessage({ type: "success", text: "Un lien magique vous a été envoyé par e-mail !" });
      }
    } catch {
        ...
        <div className="rounded-2xl border border-slate-200/60 dark:border-zinc-800/60 bg-white dark:bg-surface/90 p-8 shadow-xl shadow-slate-900/5 dark:shadow-black/30 backdrop-blur-sm">
          <div className="flex flex-col items-center text-center mb-6">
            ...
            <p className="text-xs text-slate-400 dark:text-zinc-500 mt-1">
              {isLogin ? "Trouvez les meilleures opportunités tech" : "Rejoignez les profils tech qui utilisent LinkScout"}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 mb-5">
            ...
            <span className="text-[10px] font-mono text-slate-300 dark:text-zinc-600 uppercase tracking-wider">ou continuer avec</span>
            <div className="flex-1 h-px bg-slate-100 dark:bg-zinc-800" />
          </div>
            ...
            <div className="space-y-1.5">
              <div className="flex justify-between items-center">
                <label htmlFor="password" className="text-[10px] font-mono uppercase tracking-wider text-slate-400 dark:text-zinc-500 font-bold">
                  {isLogin ? "Mot de passe" : "Choisir un mot de passe"}
                </label>
              </div>
              <input
                id="password"
          ...
          <div className="flex items-center gap-3 my-4">
            <div className="flex-1 h-px bg-slate-100 dark:bg-zinc-800" />
            <span className="text-[9px] font-mono text-slate-300 dark:text-zinc-600 uppercase tracking-wider">ou sans mot de passe</span>
            <div className="flex-1 h-px bg-slate-100 dark:bg-zinc-800" />
          </div>
          ...
          <div className="text-center space-y-2 pt-4 border-t border-slate-100 dark:border-zinc-800">
            <p className="text-xs text-slate-500 dark:text-zinc-400">
              {isLogin ? "Nouveau sur LinkScout ?" : "Déjà un compte ?"}
              <button
                type="button"
                onClick={switchMode}
